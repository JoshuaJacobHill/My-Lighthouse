...
- **Permissions**: donor and finance data needs the **`care.giving` capability**, plus the `admin/(finance)` route group. `canViewDonations` is not the gate itself — it is the extra switch a plain `ADMIN` needs before `care.giving` is granted, and it deliberately does not apply to `CARE_MANAGER` or `CHURCH_MANAGER`. Don't leak donation data to volunteer-only admins. See `docs/USERS.md`.
