...
- **Church visibility.** `User.isChurchMember` (auto-set when a user tithes — webhook + `claimDonationsForUser`; admin-toggle on the user profile via `ChurchMemberToggle`). `Story.churchOnly` + `Event.churchOnly` flags (admin form checkboxes): church-only stories are filtered on Home by `isChurchMember`; church-only events 404 for non-members. **Serving teams** ("join a team") = new `ServingTeam` + `TeamInterest` models (seeded: Creative & Worship, Host & Welcome, Kids Church). Church members see a "Serving at Lighthouse Family Church" section on the Volunteer tab (`ServingTeams` → express interest); admins manage teams + see interest at `/admin/teams` (`team.actions.ts`). Church users see all Care content too.
- **App IA / tabs.** Bottom nav = four equal tabs (Home · Volunteer · Give · Account; removed the raised centre button). **Give tab → `/donor/give`** (a hub *inside* PortalShell so the nav persists): giving total widget, Give now (→ immersive `/give/again`), recurring + tithe links, and the appeals carousel. **Home (`/donor`) refocused on Good News & updates** + two glance cards (Giving → /donor/give, Volunteering → /volunteer); the giving/appeals/volunteer depth now lives in their own tabs. Tithe form BECS Direct Debit enabled in Stripe (verify it appears on `/give/tithe`).
- **Church tithes** — `Donation.isTithe` flag. Public church-only page `/give/tithe` (direct link from lighthousefamilychurch.org.au, `robots:noindex`, not in app nav) reuses `DonateForm` targeting the church fund (CHURCH account) with `isTithe`. Tithes are **excluded from the dashboard giving total & history** (`getDonorGifts` filters `isTithe:false`). Managed at `/donor/tithes` (`tithe.actions.ts` `listMyTithes`/`updateTitheAmountAction` — in-place amount edit via new Stripe price; cancel reuses `cancelMyRecurringGift`). Dashboard shows a "Your tithe $X / freq" widget (church givers only) + an Account-settings link. Tithes are filtered out of the general `/donor/recurring` list. Webhook now also sets `frequency` on new gifts.
- **Per-fund giving amounts** — Fund.presetAmounts/suggestedAmount/impactLabels/defaultFrequency, editable in Admin → Funds, used on `/donate`.
- **Fund pages (GoFundMe-style)** at `/funds/[slug]` — image, tagline, raised-vs-goal progress, description, Give button. Logged-in Give → `/give/again?fund=<slug>` (fast on-page flow, now fund-parameterised); logged-out → `/donate?fund=<slug>`. Dashboard appeal cards now link to `/funds/[slug]`. `startGiveAgainPaymentAction` + `GiveAgainFlow` take an optional `fundSlug` (default lighthouse-care); step 1 shows a small "Giving to {fund}" line for a specific fund.
- **Recurring status labels** — `listMyRecurringGifts` now fetches `status:'all'` (was `active` only) and returns `status`/`statusLabel`/`active`/`endedAt`; `/donor/recurring` shows a status badge (Active/Payment failed/Paused/Cancelled), "cancelled {date}", and a Give-again CTA on inactive ones. Cancel action no-ops on already-cancelled subs.
- **Shout for Good historical import (CARE)** — 501 donor accounts (password-less; activate via account-setup link) + 925 gifts, original dates, `$387,139.94`, tagged `migratedFrom='shout_for_good'` (reversible), `source=OFFLINE`, `provider='SHOUT_FOR_GOOD'`. Backfilled `description`/`frequency`/`paymentMethod` (new additive Donation cols). **No emails sent.** Scripts in session scratchpad; CHURCH report (738) not yet imported. Test account: `josh+shouttest@…` (delete when done).
- **Fast "give again" flow** for logged-in donors at `/give/again` (`GiveAgainFlow.tsx`) — immersive orange amount screen (centred max-w-md; "Giving as {name}" + change link, big `$` blinking-cursor input, scrolling faint impact facts, Once/Weekly/Fortnightly/Monthly pills, NEXT) → light payment screen. Wired from the dashboard "Give again"/GivingStrip CTAs + PortalShell "Give". Defaults to Lighthouse Care (CARE).
  - **Intent-first + card-on-file + wallets.** `startGiveAgainPaymentAction` (`give.actions.ts`) derives fund/account/email from the session, ensures a reusable Stripe customer (`StripeCustomer` model, `ensureStripeCustomer` in `stripe-customer.ts`), creates the PaymentIntent (`setup_future_usage: off_session` → saves the card) or subscription up front, and returns a **CustomerSession** client secret so the Payment Element shows the donor's **saved cards** + save/remove. Step 2 also mounts an **ExpressCheckoutElement** (Apple Pay / Google Pay / Link) above the card field. Confirm is `stripe.confirmPayment({elements})` (clientSecret-based, no deferred submit). Apple Pay requires the domain registered in Stripe (done by the user, both accounts). Graceful fallback: if CustomerSession fails it returns null → plain Payment Element still works. **⚠ Needs a real-card test on prod** (saved-card display/save + Apple Pay button can't be exercised locally).
